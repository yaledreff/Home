...
def Deeplabv3(weights='pascal_voc', input_tensor=None, infer = False,
    ...
    # b4 = Lambda(lambda x: K.compat.v1.image.resize_bilinear(x,size=(int(np.ceil(input_shape[0]/OS)), int(np.ceil(input_shape[1]/OS)))))(b4)
    b4 = Lambda(lambda x: K.tf.image.resize(x,size=(int(np.ceil(input_shape[0]/OS)), int(np.ceil(input_shape[1]/OS)))))(b4)
    ...
    if backbone == 'xception':
        ...
        x = Lambda(lambda x: K.tf.image.resize(x,size=(int(np.ceil(input_shape[0]/4)), int(np.ceil(input_shape[1]/4)))))(x)
    ...
    x = Conv2D(21, (1, 1), padding='same', name=last_layer_name)(x)
    x = Lambda(lambda x: K.tf.image.resize(x,size=(input_shape[0],input_shape[1])))(x)
    if infer:
        x = Activation('softmax')(x)
    else:
        x = Reshape((input_shape[0]*input_shape[1], 21)) (x)
        x = Activation('softmax')(x)
